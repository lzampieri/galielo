...
if( array_key_exists("add", $_POST) ) {
    ...
    // Get player matches
    $query_matches = query("SELECT * FROM partite WHERE ( Att1 = $id OR Att2 = $id OR Dif1 = $id OR Dif2 = $id ) AND Hidden = 0 ORDER BY Timestamp DESC");
    $player_details["matches"] = array_from_query($query_matches);
    echo json_encode($player_details,JSON_NUMERIC_CHECK);
} else {
    // Get players details
    $query_players = query("SELECT ID, Nome, PuntiA, PuntiD, COALESCE(CountRecentA,0) AS CountRecentA, COALESCE(CountRecentD,0) AS CountRecentD, COALESCE(CountA,0) AS CountA, COALESCE(CountD,0) AS CountD FROM giocatori LEFT JOIN (SELECT Att1, Count(*) AS CountRecentA FROM (SELECT Att1, Timestamp FROM partite WHERE Hidden = 0 UNION ALL SELECT Att2, Timestamp FROM partite WHERE Hidden = 0) AS t WHERE TIMESTAMPDIFF(DAY,Timestamp,CURRENT_TIMESTAMP)<30 GROUP BY Att1) AS recentA ON giocatori.ID = recentA.Att1 LEFT JOIN (SELECT Dif1, Count(*) AS CountRecentD FROM (SELECT Dif1, Timestamp FROM partite WHERE Hidden = 0 UNION ALL SELECT Dif2, Timestamp FROM partite WHERE Hidden = 0) AS t WHERE TIMESTAMPDIFF(DAY,Timestamp,CURRENT_TIMESTAMP)<30 GROUP BY Dif1) AS recentD ON giocatori.ID = recentD.Dif1 LEFT JOIN (SELECT Att1, Count(*) AS CountA FROM (SELECT Att1, Timestamp FROM partite WHERE Hidden = 0 UNION ALL SELECT Att2, Timestamp FROM partite WHERE Hidden = 0) AS t GROUP BY Att1) AS allmatchesA ON giocatori.ID = allmatchesA.Att1 LEFT JOIN (SELECT Dif1, Count(*) AS CountD FROM (SELECT Dif1, Timestamp FROM partite WHERE Hidden = 0 UNION ALL SELECT Dif2, Timestamp FROM partite WHERE Hidden = 0) AS t GROUP BY Dif1) AS allmatchesD ON giocatori.ID = allmatchesD.Dif1 ORDER BY ID");
